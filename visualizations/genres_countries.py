...
def render_genres_countries(conn: sqlite3.Connection, year: int):
    ...
    st.header("🌍 GENRES, COUNTRIES & LANGUAGES")
    ...
    with col1:
        ...
        if not genre_df.empty:
            chart = alt.Chart(genre_df).mark_bar(color="#00c030").encode(
    ...
    with col2:
        ...
        if not country_df.empty:
            chart = alt.Chart(country_df).mark_bar(color="#40bcf4").encode(
                x=alt.X("count:Q", title="Films"),
    ...
    with col3:
        ...
        if not lang_df.empty:
            chart = alt.Chart(lang_df).mark_bar(color="#ff8000").encode(
                x=alt.X("count:Q", title="Films"),
    ...
    if runtime > 0:
        ...
        st.metric("⏱️ Total Runtime", f"{hours} hours ({days}d {remaining_hours}h)")

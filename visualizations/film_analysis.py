...
def get_rating_distribution(conn: sqlite3.Connection, year: int) -> pd.DataFrame:
    ...
    all_ratings = pd.DataFrame({
        "rating": [i * 0.5 for i in range(1, 11)]
    })
...
def render_film_analysis(conn: sqlite3.Connection, year: int):
    ...
    with col1:
        ...
        if not rating_df.empty and rating_df["count"].sum() > 0:
            chart = alt.Chart(rating_df).mark_bar(color="#00c030").encode(
            ...
            st.caption(f"⭐ Most common rating: **{top_rating['rating_str']}** stars")
        else:
    ...
    with col2:
        ...
        if not decade_df.empty:
            chart = alt.Chart(decade_df).mark_bar(color="#40bcf4").encode(
            ...
            st.caption(f"🎞️ Favorite decade: **{top_decade['decade_str']}**")
        else:
